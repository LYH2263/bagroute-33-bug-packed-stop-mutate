...
type Bag = { id: number; route_id: number; bag_index: number; weight_kg: number; volume_l: number; items: { stop_id: number; stop_name: string; weight_kg: number; volume_l: number }[] };
export default function BagsPage() {
  ...
  return (<>
    ...
    <tbody>{rows.map(b => <tr key={b.id}><td>{b.route_id}</td><td>{b.bag_index}</td><td className="mono">{b.weight_kg}</td><td className="mono">{b.volume_l}</td>
      <td>{b.items.map(i => `${i.stop_name}（${i.weight_kg}kg / ${i.volume_l}L）`).join(" → ")}</td></tr>)}
